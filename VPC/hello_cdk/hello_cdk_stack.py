...
class ByeCdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        vpc = ec2.Vpc(self, "TheVPC", ip_addresses=ec2.IpAddresses.cidr("10.0.0.0/16"),
                      max_azs=2, subnet_configuration=[{"cidrMask": 24, "name":"public", "subnetType": ec2.SubnetType.PUBLIC},
        ...
        mysqlInstance = rds.DatabaseInstance(self, "mysqlInstnace", engine=rds.DatabaseInstanceEngine.MYSQL,
                                             vpc=vpc, instance_type=ec2.InstanceType.of(ec2.InstanceClass.BURSTABLE2, ec2.InstanceSize.MICRO), 
                                             security_groups=[mysqlSG], removal_policy=cdk.RemovalPolicy.DESTROY, allocated_storage=20,
                                             database_name="MyDB", vpc_subnets=ec2.SubnetSelection(subnet_type=ec2.SubnetType.PRIVATE_ISOLATED),
                                             credentials=rds.Credentials.from_password(username="admin", password=cdk.SecretValue.unsafe_plain_text("password123")))
        ...
        template = ec2.LaunchTemplate(self,"LaunchTemplate", machine_image=ec2.MachineImage.latest_amazon_linux2023(),
                                      security_group=autoscalingSG, instance_type=ec2.InstanceType.of(ec2.InstanceClass.BURSTABLE2, ec2.InstanceSize.MICRO),
                                      user_data=ec2.UserData.add_execute_file_command(self, file_path="./script/user-data.sh", arguments=endpoint))
        ...
        autog = autoscaling.AutoScalingGroup(self, "ASG", vpc=vpc, launch_template=template, auto_scaling_group_name="MyASG",
                                             ...
                                             max_capacity=4, min_capacity=1, notifications=[autoscaling.NotificationConfiguration(topic=topic)])
